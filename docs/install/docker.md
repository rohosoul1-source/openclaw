...
4. Persist generated n8n workflow JSON files in a shared archive path (for
   example `/home/node/.openclaw/workspace/n8n/workflow-archive`) so they can
   be imported/replayed locally by n8n and referenced by OpenClaw.
5. Put operational runbooks and workflow contracts in workspace markdown files
   that skills can read (for example `workspace/notes/workflows.md`).
6. Keep `models.defaults.model.primary` pointed at your local 7B Ollama model so
...
- Save each generated workflow JSON to `/home/node/.openclaw/workspace/n8n/workflow-archive/*.json`.
- Import and execute workflows from that archive path in local n8n runs.
...
This keeps orchestration auditable: workflow policy lives in markdown, workflow
artifacts live in a versionable JSON archive, and execution remains in OpenClaw
skills that call n8n on the internal Docker network.
